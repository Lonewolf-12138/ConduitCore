 ...
 * @date 2023/5/21 15:55
 * @description TODO
 */

public class NumberTextFieldWidget extends ClickableWidget implements Drawable, Element {
    ...
    private long renderTipTick = 0;

    private String tipText = "";

    public NumberTextFieldWidget(Screen parentScreen, float maxNumber, float minNumber, TextRenderer textRenderer, int x, int y, int width, int height, Text text) {
        this(parentScreen,maxNumber, minNumber, textRenderer, x, y, width, height, (TextFieldWidget)null, text);
    }

    public NumberTextFieldWidget(Screen parentScreen, float maxNumber, float minNumber, TextRenderer textRenderer, int x, int y, int width, int height, @Nullable TextFieldWidget copyFrom, Text text) {
...
//        this.text = text.asString();
    ...
    public void render(MatrixStack matrices, int mouseX, int mouseY, float delta) {
        if (this.visible) {
            ...
            if(this.renderTipTick > System.currentTimeMillis()){
                parentScreen.renderTooltip(matrices, Text.of(tipText), offX + width, offY + height / 2);
            }else if(isActive()) {
                parentScreen.renderTooltip(matrices, Text.of("§c数值范围: " + minNumber + " ~ " + maxNumber), offX + width, offY + height / 2);
            }
        }
    }
    ...
    public void write(String writeText) {
        String regex = "[0-9-.]+";
        if (!Pattern.matches(regex, writeText)) {
            this.renderTipTick = System.currentTimeMillis() + 1000;
            this.tipText = "§c只能写入数字和小数点和负号";
            return;
        }
        int i = Math.min(this.selectionStart, this.selectionEnd);
        int j = Math.max(this.selectionStart, this.selectionEnd);
        int k = this.maxLength - this.text.length() - (i - j);
        String string = SharedConstants.stripInvalidChars(writeText);
        int l = string.length();
        if (k < l) {
    ...
    public float getNumber(Float defaultNumber){
        try {
            float number = Float.parseFloat(this.text);
